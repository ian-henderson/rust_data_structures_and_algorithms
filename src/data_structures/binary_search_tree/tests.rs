...
fn basics() {
    ...
    // Check empty BinarySearchTree behaves correctly
    ...
    // Populate BinarySearchTree with one element
    let is_duplicate = tree.insert(1);
    assert_eq!(is_duplicate, false);

    // Check one-element BinarySearchTree behaves correctly
    ...
    assert!(tree.insert(0)); // duplicate 

    assert_eq!(tree.size, 9);
    assert_eq!(tree.root(), Some(&1));
    assert_eq!(
        tree.sorted_vec(),
        vec![&-5, &-2, &0, &0, &1, &3, &5, &5, &15]
    );
...
fn insert_without_duplication() {
    let mut tree: BinarySearchTree<i32> = BinarySearchTree::new();
    assert_eq!(tree.insert_without_dup(0), false);
    assert_eq!(tree.insert_without_dup(1), false);
    assert_eq!(tree.insert_without_dup(0), true);
...
fn successor_and_predecessor() {
    ...
    assert_eq!(tree.successor(&18), Some(&25));
    assert_eq!(tree.successor(&40), Some(&45));
    assert_eq!(tree.successor(&10), Some(&15));
    ...
    assert_eq!(
        tree.sorted_vec(),
        vec![&10, &10, &15, &18, &25, &25, &35, &35, &40, &45, &45]
    );
}
...
fn from_iter() {
    ...
    let tree: BinarySearchTree<i32> =
        BinarySearchTree::from_iter(vec![7, 1, 0, 4, 5, 3].into_iter());
    assert_eq!(tree.len(), 6);
    assert_eq!(tree.sorted_vec(), [&0, &1, &3, &4, &5, &7]);
...
fn reverse_order() {
    ...
    let tree: BinarySearchTree<i32> = vec![7, 1, 0, 4, 2, 5, 3, 6, 8].into_iter().collect();
    assert_eq!(
        tree.reverse_order().collect::<Vec<&i32>>(),
...
fn preorder() {
    ...
    let tree: BinarySearchTree<i32> = vec![7, 1, 0, 4, 2, 5, 3, 6, 8].into_iter().collect();
    assert_eq!(
        tree.preorder().collect::<Vec<&i32>>(),
        vec![&7, &1, &0, &4, &2, &3, &5, &6, &8]
    );
    assert_eq!(tree.sorted_vec(), vec![&0, &1, &2, &3, &4, &5, &6, &7, &8]);
}
...
fn postorder() {
    ...
    let tree: BinarySearchTree<i32> = vec![7, 1, 0, 4, 2, 5, 3, 6, 8].into_iter().collect();
    assert_eq!(
        tree.postorder().collect::<Vec<&i32>>(),
        vec![&0, &3, &2, &6, &5, &4, &1, &8, &7]
    );
    assert_eq!(tree.sorted_vec(), vec![&0, &1, &2, &3, &4, &5, &6, &7, &8]);
}
...
fn level_order() {
    ...
    let tree: BinarySearchTree<i32> = vec![7, 1, 0, 4, 2, 5, 3, 6, 8].into_iter().collect();
    assert_eq!(
        tree.level_order().collect::<Vec<&i32>>(),
        vec![&7, &1, &8, &0, &4, &2, &5, &3, &6]
    );
    assert_eq!(tree.sorted_vec(), vec![&0, &1, &2, &3, &4, &5, &6, &7, &8]);
}
...
fn partail_eq() {
    ...
    let first: BinarySearchTree<String> = vec!["abc".to_string(), "efg".to_string()]
        .into_iter()
        .collect();
    let second: BinarySearchTree<String> = vec!["abc".to_string(), "efg".to_string()]
        .into_iter()
        .collect();
    assert!(first == second);
...
fn display() {
    let tree: BinarySearchTree<i32> = vec![7, 1, 0, 4, 2, 5, 3, 6, 8].into_iter().collect();
    println!("{}", tree);
    let tree: BinarySearchTree<String> = vec!["abc".to_string(), "efg".to_string()]
        .into_iter()
        .collect();
    println!("{}", tree);
}
